...
function hasThreeOrMoreMatchingLetters(word1, word2) {
  const minLength = Math.min(word1.length, word2.length);
  let matchCountLeft = 0;
  let matchCountRight = 0;
  ...
  return matchCountLeft >= 3 || matchCountRight >= 3;
}

const word = getUrlParameter('word').toLowerCase();
...
if (wordDictionary.hasOwnProperty(word)) {
  const currentExplanation = wordDictionary[word].explanation;
  explanationText.innerHTML = styleCapitalWords(getFirstFiveLines(currentExplanation));
  text.textContent = word;
  audioPlayer.src = wordDictionary[word].audioSrc;
  explanationDiv.classList.remove('hidden');

  // Find and display explanations with matching criteria
  Object.keys(wordDictionary).forEach(key => {
    if (key !== word && hasThreeOrMoreMatchingLetters(word, key)) {
      const matchingExplanation = wordDictionary[key].explanation;

      const wordDiv = document.createElement('div');
      wordDiv.classList.add('mx-auto', 'lg:w-[700px]');
      wordDiv.innerHTML = `
        <p class="capitalize text-xl md:text-xl lg:text-xl text-sky-500 mt-2">${key}</p>
        <p class="text-gray-700 mx-auto text-sm md:text-sm lg:text-base mt-1">${styleCapitalWords(getFirstFiveLines(matchingExplanation))}</p>
      `;
      explanationDiv.appendChild(wordDiv);

      const matchingAudioPlayer = document.createElement('audio');
      matchingAudioPlayer.src = wordDictionary[key].audioSrc;
      wordDiv.appendChild(matchingAudioPlayer);
    }
  });

} else {
  alert("kenya lentsoe kapa kenya le nepahetseng.");
}

function getFirstFiveLines(text) {
  const lines = text.split('\n');
  return lines.slice(0, 5).join('\n');
}

function styleCapitalWords(text) {
